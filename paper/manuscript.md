...
The *Drosophila* mushroom body (MB) has been modeled for thirty years as a single-channel reward-prediction-error (RPE) machine. Recent imaging shows dopaminergic neurons (DANs) also respond to novel odors before any reinforcement (Hattori 2017), and behavioural data show latent inhibition (Jacob & Waddell 2022) and fast reversal (Mancini 2019) — phenomena single-channel RPE cannot explain. Using the recently-released larval connectome (Winding 2023; 3,016 neurons, every synapse mapped), we built three computational agents on identical wiring: classical RPE (Bennett 2021), a Bayesian-observer "AIF" channel tracking perceptual surprisal, and two hybrid architectures. Across four behavioural protocols on the same connectome substrate, each architecture is *individually necessary*: signed-weight RPE is required for extinction; a Bayesian surprisal channel is required for novelty signaling and latent inhibition; parallel valence traces (Bennett-MV / Felsenberg-POM) are required for reversal at acquisition speed. No two-of-four combination matches all four signatures. Clustering MBINs by shared KC pools recovers the canonical Eichler 2017 compartmental organization (DAN-i1 ↔ MBON-i1 from wiring alone, Jaccard=0.82). We make a specific empirical prediction: the larval DAN carrying perceptual surprise should habituate to novel odors before reinforcement, testable by calcium imaging in DAN-c1 (the candidate by analogy to adult PPL1-α'3).

---

## Introduction *(target 800 words)*

Three paragraphs, in order:

1. **The MB-as-RPE consensus**: 30 years of evidence (Tully/Quinn → Heisenberg → Bennett 2021), DAN-gated KC→MBON plasticity, beautifully simple. Cite Modi 2020 review.
2. **The cracks in the consensus**: α'3 novelty firing (Hattori 2017), parallel opposing memories (Felsenberg 2018), latent inhibition (Jacob & Waddell 2022), fast reversal (Mancini 2019). What pure RPE *cannot* explain.
3. **What the connectome enables now**: with Winding 2023 we can finally test which architectural ingredients are individually necessary, by holding the wiring fixed and varying only the learning rule. This paper isolates four such ingredients across four behavioural protocols.
